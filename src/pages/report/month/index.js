...
const Http = WowPage.wow$.plugins.http;
const Modal = WowPage.wow$.plugins.modal;
...
WowPage({
    ...
    onLoad () {
        this.userGet();
        type = false;
        this.setData({ cTime: formatData('yyyy-MM')});
        this.initData('-1');
        this.assignmentData();
        this.getMonthReport();
        this.reqSugarSpider();
    ...
    getMonthReport () {
        ...
        let weekReport;
        return Http(Http.API.REQ_REPORT_MONTH_INFO, {
            Stime,
            Etime,
        }).then((res) => {
            weekReport = res || {};
    ...
    reqMonthlyTrend () {
        ...
        let Etime = this.data.eTime;
        let weekData;
        return Http(Http.API.REQ_SUGAR_TREND, {
            Stime,
            Etime,
        }).then((res) => {
            weekData = res || [];
    ...
    reqSugarSpider () {
        ...
        let Etime = this.data.eTime;
        let objRadar;
        return Http(Http.API.REQ_SUGAR_SPIDER, {
            Stime,
            Etime,
        }).then((res) => {
            objRadar = res || {};
